...
open class LoginCoordinator: ConfigurationSource {
    ...
    public lazy var twitterService = TwitterService()
    ...
    public lazy var passwordService = PasswordRecoverService()
    ...
    open func login(profile:LoginProfile) {
        print("Implement this method in your subclass to handle login.")
    }
    
    open func loginError(error:NSError) {
        print("Implement this method in your subclass to handle login.")
    }
    
    open func signUpError(error:NSError) {
        print("Implement this method in your subclass to handle login.")
    }

    open func signup(profile:SignUpProfile) {
    ...
    open func recoverPassword(success: Bool) {
        print("Implement this method in your subclass to handle password recovery.")
    }
    
    open func recoverPasswordError(error: NSError) {
        print("Implement this method in your subclass to handle password recovery.")
    }
}
...
extension LoginCoordinator: InitialViewControllerDelegate {
    ...
    func didSelectTwitter(_ viewController: UIViewController) {
        twitterService.login(from: viewController) { (result) in
            switch result {
            case .success(let profile):
                self.enterWithTwitter(profile: profile)
                break
            case .error(let error):
                self.loginError(error: error as NSError)
                break
...
extension LoginCoordinator: LoginViewControllerDelegate {
    ...
    func didSelectLogin(_ viewController: UIViewController, email: String, password: String) {
        loginService.login(from: viewController, username: email, password: password) { (result) in
            switch result {
            case .success(let profile):
...
extension LoginCoordinator: SignupViewControllerDelegate {
    ...
    func didSelectSignup(_ viewController: UIViewController, user: PFUser) {
        signUpService.login(from: viewController, user: user){ (result) in
            switch result {
            case .success(let profile):
                self.signup(profile: profile)
                break
            case .error(let error):
                self.signUpError(error: error as NSError)
                break
            default:
                break
            }
...
extension LoginCoordinator: PasswordViewControllerDelegate {
    ...
    func didSelectRecover(_ viewController: UIViewController, email: String) {
        passwordService.resetPassword(from: viewController, emailAddress: email) { (result) in
            switch result {
            case .success(let success):
                self.recoverPassword(success: success)
                break
            case .error(let error):
                self.recoverPasswordError(error: error as NSError)
                break
            default:
                break
            }
...
enum Font: String {
    ...
    func get(size: CGFloat = 15.0) -> UIFont {
        if self.rawValue != nil && size != nil
        {
            return UIFont(name: self.rawValue, size: size)!
        }
        return UIFont(name: "Montserrat-Regular", size: CGFloat(13.0))!
    }
